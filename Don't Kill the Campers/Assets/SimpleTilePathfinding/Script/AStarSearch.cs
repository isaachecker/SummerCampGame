...
namespace SimplePF2D {
    ...
    /*
     * Summary:
     * The starting point gets all adjacent points and calculates their f value
     * f value is the sum of the g and h values
     * g value is a combination of the parent directional cost, current directional cost, and movement penalty
     * h value is the manhattan distance to the end
     * Based on the f value, the current node is added to a index in the open bin, where the lower the f, the better the position
     * Then we grab the best node from the open bin, and do that over and over again
     */
    ...
    public class AStarSearch{
        ...
        private NavWrapper CreateNavWrapper(NavNode node)
        {
            ...
            { // Create a new one or dequeue a free one from the navwrapper pool
                if (freeList.Count > 0)
                {
        ...
        {
            //TODO would it be faster to cache which index has the highest value?
            for (int i = 0; i < maxbins; i++)
            {
        ...
        {
            int xDist = Mathf.Abs(currentNode.GetIndex().x - endWrapper.node.GetIndex().x);
            int yDist = Mathf.Abs(currentNode.GetIndex().y - endWrapper.node.GetIndex().y);
            return xDist + yDist;
        ...
        private bool CreatePath(NavNode startNode, NavNode endNode){
            ...
            {
                Debug.LogError("The start and/or end tile are invalid");
