...
public class SimplePathFinding2D : MonoBehaviour {
    ...
    private int navMinX = 0; // These are the max and min world coordinates of the nav grid.
    private int navMinY = 0;
    ...
    private void CreateNavMesh()
    {
        ...
        for (int i = 0; i < ColliderTileMaps.Count; i++)
        {
            for (int x = 0; x < w; x++)
            {
                for (int y = 0; y < l; y++)
                {
                    pos.x = x;
                    pos.y = y;
                    pos.z = Controls.GetTileZpf();
                    //TODO set a specific "z" to be the "blockTile" z value
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < l; y++) {
                ...
                { // Make the edges of the grid blocked.
                    SelectedNavigationTileMap.SetTile(NavToTile(pos), BlockTile);
                ...
                else
                {
                    ...
                    else if (tile != null)
                    {
                        ...
                        navNodes[x, y].SetBlocked(false);  //set explicitly for debugging purposes
                    }
                }
    ...
    public void UpdateNavMesh(BoundsInt bounds)
    {
        Vector3Int pos = new Vector3Int();
        ...
        for (int x = bounds.xMin; x < bounds.xMax; x++)
        {
            for (int y = bounds.yMin; y < bounds.yMax; y++)
            {
                ...
                for (int i = 0; i < ColliderTileMaps.Count; i++)
                {
                    if (ColliderTileMaps[i].GetTile(pos) != null)
                    {
                        SelectedNavigationTileMap.SetTile(pos, BlockTile);
                        navNodes[x, y].SetBlocked(true);
                    }
                }

                TileBase tile = SelectedNavigationTileMap.GetTile(pos);
                if (tile != BlockTile && tile != null)
                {
                    if (NavigationMovementPenaltyTiles.Count == CorrespondingMovementPenalty.Count)
                    {
                        // Loop through and add movement penalties to all the user added tiles.
    ...
    public void RemoveOneBlockedPoint(Vector3Int pos)
    {
        navNodes[pos.x, pos.y].SetBlocked(false);
    }
    ...
    void Update()
    {
        ...
        if (pathqueries.Count > 0){
            ...
            {
                float qratio = 1.0f - ((pathqueries.Count - 1) / PathLoad);
